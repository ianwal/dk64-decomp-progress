...
class Converter():
    def __init__(self, mode, infile, outfile, *argv):
        ...
        if mode == "auto":
            # Extract the image format (rgba16, etc.) from a filename such as x.rgba16.png
            inpath = Path(infile)
            inpath_suffixes = inpath.suffixes
            if len(inpath_suffixes) < 2:
                print(f"could not determine format for image {infile}", file=stderr)
                exit(1)
            img_format = inpath_suffixes[-2][1:]
            if not img_format in valid_image_formats:
                print(f"invalid format for image {infile}: {img_format}", file=stderr)
                exit(1)
